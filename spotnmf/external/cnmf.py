...
class cNMF():
    ...
    def prepare(self, input_counts, components, n_iter = 100, tpm_fn=None, seed=None,
                        beta_loss='frobenius', num_highvar_genes=2000,
        ...
        #     elif counts_fn.endswith('.csv'):
        #         input_counts = pd.read_csv(counts_fn, sep=',', index_col=0)
        #         input_counts = input_counts.astype(float)
        #     else:
        #         input_counts = pd.read_csv(counts_fn, sep='\t', index_col=index_col)
        #         input_counts = input_counts.astype(float)
        ...
        highvargenes = input_counts.var.index[input_counts.var["highly_variable"]].tolist()
    ...
    def factorize(self,
                worker_i=0, total_workers=1, method_type = "nmf",
                ):
        """
        ...
        for idx in jobs_for_this_worker:
            ...
            if(method_type == "nmf"):
                (spectra, usages) = self._nmf(norm_counts.X, _nmf_kwargs) ## TODO make factorize with nmf or OT
            elif(method_type == "ot"):
                (spectra, usages) = self._otmf(norm_counts, components=p['n_components'])
                print(spectra.shape)
    ...
    def refit_usage(self, normcounts, spectra, method_type):
        """
    ...
    def refit_spectra(self, tpm, usage, method_type):
        """
    ...
    def consensus(self, k, density_threshold=0.5, local_neighborhood_size = 0.30, show_clustering = True,
                  ...
                  refit_usage=True, method_type='nmf'):
        """
        ...
        if not skip_density_and_return_after_stats:
            ...
            ## Fixed threshold
            l2_spectra = l2_spectra.loc[local_density.index] # align it
        ...
        try:
            ...
            print(f'Kmeans error at {k} :{ex}')
            kmeans_model = KMeans(n_clusters=len(l2_spectra), n_init=10, random_state=1)
            kmeans_model.fit(l2_spectra)
        ...
        spectra_tpm = self.refit_spectra(tpm, norm_usages, method_type)
        ...
        if refit_usage:
            ...
            rf_usages = self.refit_usage(norm_tpm, spectra_tpm_rf, method_type)
...
def run_cnmf(
    ...
    Parameters
    ----------
    adata_spatial : AnnData or compatible object
        Input spatial data (gene expression counts).
    components : list[int]
        List of component numbers (k values) to use for NMF.
    n_genes : int, default=2000
        Number of highly variable genes to use.
    nprocess : int, default=8
        Number of parallel processes.
    temp_dir : str, optional
        Directory for temporary results. If None, a temporary directory is created.
    method_type : str, default='nmf'
        Factorization method to use.
    n_iter : int, default=200
        Number of iterations for NMF.
    sample_name : str, optional
        Name of the dataset/sample. If None, tries to get from `adata_spatial.uns['dataset_name']`.

    Returns
    -------
    dict
        Dictionary containing:
        - "topics_per_spot": usage matrix
        - "genes_per_topic": spectra scores
        - f"spectra_tpm_c{method_type}": TPM per topic
        - f"top_genes_c{method_type}": top genes per topic
    """
    temp_dir_obj = None
    try:
        ...
        if sample_name is None:
            sample_name = adata_spatial.uns.get("dataset_name", "sample")
        ...
        # Run consensus for each component
        for k in components:
            try:
                print(f'Starting consensus for k={k}')
                cnmf_obj.consensus(k=k, density_threshold=0.10, show_clustering=False, method_type=method_type)
            except Exception as ex:
