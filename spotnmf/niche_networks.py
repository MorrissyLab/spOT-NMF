...
def generate_node_attributes(usage: pd.DataFrame, usage_threshold: Union[int, float], cond1: str):
    """
...
def detect_communities_infomap(graph: Union[nx.Graph, nx.DiGraph]) -> Union[nx.Graph, nx.DiGraph]:
    ...
    # Convert to list of tuples with weights
    ...
    print(f"Detected {len(communities)} communities using Infomap.")
    ...
    # Now build new graph with names as nodes
    graph_nx = nx.DiGraph()
    graph_nx.add_nodes_from(names)
    for source, target, weight, weight_col in edge_tuples:
        graph_nx.add_edge(source, target, weight=weight, weight_col=weight_col)

    # Assign cluster memberships
    nx.set_node_attributes(graph_nx, mapping, "cluster")
...
def plot_connection_heatmap(group_connections, rowannot=None, columnannot=None,
                            ...
                            suptitle="In-group and Out-group Connection Heatmap", legend_title="n.edges"):
    ...
    g = sns.clustermap(
        ...
        xticklabels=True,
        yticklabels=True,
        dendrogram_ratio=(.1, .1),  # optional: reduce dendrogram size
        cbar_pos=(1, 0.3, 0.05, 0.2)  # optional: move colorbar
    )

    # Function to create a legend
    def create_cluster_legend(cluster_labels, color_map, title):
    ...
    # Create legends
    row_legend_handles = create_cluster_legend(row_clusters, cluster_color_map, "Row Clusters") if row_clusters is not None else []
